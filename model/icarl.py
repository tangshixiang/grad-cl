...
class Net(torch.nn.Module):
    ...
    def __init__(self, n_inputs, n_outputs, n_tasks, args):
        ...
        # setup network
        assert (args.data_file == 'cifar100.pt') or (
            args.data_file == 'tiny-imagenet-200.pt') or (
                args.data_file == 'cifar10.pt')

        if args.data_file == 'cifar100.pt' or args.data_file == 'cifar10.pt':
            self.net = ResNet18(n_outputs)
        elif args.data_file == 'tiny-imagenet-200.pt':
    ...
    def forward(self, x, t):
        ...
        if t * self.nc_per_task not in self.mem_class_x.keys():
            ...
            out[:,
                int(t * self.nc_per_task):int((t + 1) *
                                              self.nc_per_task)].fill_(
        ...
        if self.num_exemplars > 0:
            ...
            for tt in range(t):
                ...
                loss += self.reg * self.kl(
                    ...
                    self.sm(
                        target_dist[:, offset1:offset2])) * self.nc_per_task
        # bprop and update
        loss.backward()
        self.opt.step()
