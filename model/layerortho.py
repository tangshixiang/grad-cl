...
def project2cone2(gradient, memories, beg, en, margin=0.5, eps=1e-3):
    """
...
def project3cone3(gradient, memories, beg, en, margin=0.5, eps=1e-3):
    """
    ...
    memories_np = memories[beg:en].cpu().t().double().numpy()
    gradient_np = gradient[beg:en].cpu().contiguous().view(-1).double().numpy()
    memories_np_orth = orth(memories_np.transpose()).transpose()

    x = gradient_np
    params = []
    for i in range(len(memories_np_orth)):
        param = np.dot(memories_np_orth[i], x) / np.dot(
            memories_np_orth[i], memories_np_orth[i].transpose())
        x = x - np.min([param, -1 * margin]) * memories_np_orth[i].reshape(-1)
        params.append(-np.min([param, -1 * margin]))
...
class Net(nn.Module):
    ...
    def observe(self, x, t, y):
        ...
        if len(self.observed_tasks) > 1:
            ...
            for cnt in range(len(self.grad_dims)):
                beg = 0 if cnt == 0 else sum(self.grad_dims[:cnt])
                en = sum(self.grad_dims[:cnt + 1])
                dotp = torch.mm(self.grads[beg:en, t].unsqueeze(0),
