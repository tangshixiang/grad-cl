...
def project5cone5(gradient, memories, margin=0.5, eps=1e-3):
    """
    ...
    # memories_np_pca = PCA(n_components=len(memories_np))
    # memories_np_pca.fit(memories_np_del_mean)
    if len(memories_np) == 1:
        x = gradient_np - (memories_np_mean.transpose().dot(gradient_np) /
                           memories_np_mean.transpose().dot(memories_np_mean)
                           ) * memories_np_mean
    else:
        ...
        Pg_bar = memories_np_mean - memories_np_orth.transpose().dot(
            memories_np_orth.dot(memories_np_mean))
        if memories_np_mean.transpose().dot(Pg) > 0:
            x = Pg
        else:
            x = Pg - (memories_np_mean.transpose().dot(Pg) /
                      memories_np_mean.transpose().dot(Pg_bar)) * Pg_bar
    gradient.copy_(torch.Tensor(x).view(-1, 1))


def project6cone6(gradient, memories, margin=0.5, eps=1e-3):
    """
        Solves the GEM dual QP described in the paper given a proposed
        gradient "gradient", and a memory of task gradients "memories".
        Overwrites "gradient" with the final projected update.
...
class Net(nn.Module):
    ...
    def observe(self, x, t, y):
        ...
        if len(self.observed_tasks) > 1:
            ...
            if True:
                project5cone5(self.grads[:, t].unsqueeze(1),
