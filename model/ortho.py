...
def project3cone3(gradient, memories, margin=0.5, eps=1e-3):
    """
    ...
    memories_np = memories.cpu().t().double().numpy()
    gradient_np = gradient.cpu().contiguous().view(-1).double().numpy()
    memories_np_orth = orth(memories_np.transpose(), rcond=eps).transpose()
    x = gradient_np
    params = []
...
class Net(nn.Module):
    ...
    def observe(self, x, t, y):
        ...
        if len(self.observed_tasks) > 1:
            ...
            if (dotp < 0).sum() != 0:
                project3cone3(self.grads[:, t].unsqueeze(1),
