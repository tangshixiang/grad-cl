...
class Continuum:
    def __init__(self, data, args):
        ...
        for t in range(n_tasks):
            ...
            for epoch in range(args.n_epochs):
                task_p = [[task_t, i, epoch]
                          for i in sample_permutations[task_t]]
    ...
    def __next__(self):
        if self.current >= self.length:
            ...
            j = []
            i = 0
            ep = self.permutation[self.current][2]
            while (((self.current + i) < self.length)
            ...
            return self.data[ti][1][j], ti, self.data[ti][2][j], ep
...
def life_experience(model, continuum, x_te, args):
    ...
    for (i, (x, t, y, ep)) in enumerate(continuum):
...
if __name__ == "__main__":
    ...
    parser.add_argument('--data_file',
                        default='mnist_permutations.pt',
                        help='data file')
    parser.add_argument('--samples_per_task',
                        type=int,
                        default=-1,
    ...
    if hasattr(model, 'first_loss'):
        ...
        np.save(fname + '.first_loss', first_loss)
        np.save(fname + '.all_loss', all_loss)
    # save all results in binary file
    torch.save(
        (result_t, result_a, model.state_dict(), stats, one_liner, args),
        fname + '.pt')
